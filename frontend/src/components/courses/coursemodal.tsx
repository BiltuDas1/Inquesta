...
export default function CourseModal({ editing, onClose, onSave, isSubmitting }: ModalProps) {
  ...
  const [preview, setPreview] = useState<string | null>(
    editing?.icon ? `${import.meta.env.VITE_SUPERBASE_PUBLIC_URL}/${editing.icon}` : null
  );
  ...
  return (
    <div className="fixed inset-0 bg-black/70 z-[100] flex items-center justify-center p-4 backdrop-blur-sm" onClick={(e) => e.target === e.currentTarget && onClose()}>
      <div className="bg-[#1c2026] border border-[#3b4a44] rounded-[20px] p-8 w-full max-w-[950px] max-h-[90vh] overflow-y-auto font-body shadow-2xl" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <select name="level" className={inputClass} value={formData.level} onChange={handleChange}>
                  {LEVELS.map((l) => <option key={l} value={l} className="bg-[#1c2026]">{l}</option>)}
                </select>
              </div>
              <div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <input name="duration" className={inputClass} type="text" value={formData.duration} onChange={handleChange} />
              </div>
              <div>
          ...
          <div className="flex flex-col gap-4">
            <div>
              ...
              <div onClick={() => fileInputRef.current?.click()} className="w-full h-32 bg-[#262a31] border border-dashed border-[#3b4a44] rounded-[10px] flex items-center justify-center cursor-pointer hover:border-[#6fffd9] overflow-hidden transition-colors">
                {preview ? (
                  ...
                  <div className="flex flex-col items-center text-[#84948e]">
                    ...
                    <span className="text-[0.75rem] font-headline font-semibold">Upload Image</span>
                  </div>
                )}
              </div>
