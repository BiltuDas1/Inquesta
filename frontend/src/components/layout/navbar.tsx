    ...
    <>
      ...
      <nav className="sticky top-0 z-50 text-on-surface font-headline flex items-center h-16 px-4 bg-[#181c22] border-b border-[#3b4a44] w-full">
        ...
        <div className="flex md:hidden items-center justify-between w-full">
          ...
          <div className="flex items-center gap-1">
            <button onClick={() => setIsMobileSearchOpen(true)} className={iconBtnClass}>
              <span className="material-symbols-outlined" style={{ fontSize: "25px", fontVariationSettings: "'wght' 300" }}>search</span>
            </button>
            ...
            {user ? (
                ...
                {isProfileOpen && (
                  <div className="absolute right-0 mt-2 w-48 bg-[#1c2026] border border-[#3b4a44] rounded-xl shadow-xl py-2 z-[60] animate-in fade-in zoom-in duration-150">
                    <div className="px-4 py-2 border-b border-[#3b4a44] mb-1">
                      <p className="text-[#dfe2eb] text-sm font-bold truncate">{user.firstname} {user.lastname}</p>
                      <p className="text-[#84948e] text-xs truncate">{user.email}</p>
                    </div>
                    <button onClick={handleLogout} className="w-full text-left px-4 py-2 text-[#ffb4ab] hover:bg-[#262a31] text-sm transition-colors flex items-center gap-2">
              ...
              <button onClick={() => navigate("/login")} className="px-3 py-1.5 rounded-lg bg-[#6fffd9] text-[#1c2026] font-bold text-xs hover:bg-[#5cebc5] transition-colors shrink-0 ml-2">Login</button>
            )}
          </div>
        </div>
        ...
        <div className="hidden md:flex items-center w-full justify-center gap-2 lg:gap-45">
          ...
          <div className="flex-1 min-w-[200px] max-w-[640px] relative mx-2 lg:mx-6">
            ...
            <input
              ...
              className={`w-full py-2 pr-4 pl-10 rounded-full bg-[#1c2026] outline-none text-[#dfe2eb] text-sm transition-all duration-200 border ${
                searchFocused ? "border-[#6fffd9] shadow-[0_0_0_3px_rgba(111,255,217,0.12)]" : "border-[#3b4a44]"
              }`}
            />
          </div>
          ...
          <div className="flex items-center gap-1 shrink-0">
            {user ? (
                ...
                {isProfileOpen && (
                  <div className="absolute right-0 top-[calc(100%+10px)] w-56 bg-[#1c2026] border border-[#3b4a44] rounded-xl shadow-2xl py-2 z-[60] animate-in fade-in zoom-in duration-150">
                    <div className="px-4 py-3 border-b border-[#3b4a44] mb-2">
                      <p className="text-[#dfe2eb] text-sm font-bold">{user.firstname} {user.lastname}</p>
                      <p className="text-[#84948e] text-xs truncate">{user.email}</p>
                    </div>
                    <button onClick={handleLogout} className="w-full text-left px-4 py-2 text-[#ffb4ab] hover:bg-[#262a31] text-sm transition-colors flex items-center gap-2 font-medium">
              ...
              <button onClick={() => navigate("/login")} className="px-5 py-2 rounded-lg bg-[#6fffd9] text-[#1c2026] font-bold text-sm hover:bg-[#5cebc5] transition-colors shrink-0 ml-2">Login</button>
            )}
          </div>
        </div>
      </nav>
      ...
      {isMobileSearchOpen && (
        <div className="fixed inset-0 z-[200] bg-[#181c22] flex flex-col md:hidden">
          <div className="flex items-center px-4 h-16 border-b border-[#3b4a44] gap-3 shrink-0">
            <span className="material-symbols-outlined text-[#84948e]" style={{ fontSize: "22px" }}>search</span>
            <input type="text" placeholder="Search for anything" autoFocus className="flex-1 bg-transparent border-none outline-none text-[#dfe2eb] text-base" />
            <button onClick={() => setIsMobileSearchOpen(false)} className="bg-transparent border-none cursor-pointer text-[#b9cac3]"><span className="material-symbols-outlined">close</span></button>
          </div>
        </div>
      )}
    </>
