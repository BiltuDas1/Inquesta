...
import { gql } from "@apollo/client"; // <-- Import Apollo hooks
            ...
            if (item.subItems) {
              ...
              return (
                <div key={item.name} className="flex flex-col">
                  ...
                  <div
                    className={`overflow-hidden transition-all duration-300 flex flex-col space-y-1 ${
                      isExpanded && !isDesktopCollapsed ? "max-h-40 mt-1" : "max-h-0"
                    }`}
                  >
                    {item.subItems.map((sub) => {
                      const isSubActive = currentUrl.startsWith(sub.path);
        ...
        <header className="h-16 flex items-center justify-between px-4 sm:px-6 lg:px-8 bg-[#1c2026] border-b border-[#3b4a44] shrink-0 z-10 transition-all duration-300">
          ...
          <div className="flex items-center gap-2 sm:gap-4 relative">
            ...
            <div className="relative" ref={notificationRef}>
              ...
              {/* Pass the dynamically fetched notifications here */}
              <NotificationModal 
            ...
            <div className="relative" ref={profileRef}>
              <button
                onClick={() => setIsProfileOpen(!isProfileOpen)}
                className="w-8 h-8 sm:w-10 sm:h-10 rounded-full bg-[#3b4a44] border-2 border-[#6fffd9] flex items-center justify-center overflow-hidden cursor-pointer shadow-[0_0_10px_rgba(111,255,217,0.2)] hover:border-[#5cebc5] transition-colors focus:outline-none"
              >
                {user ? (
                  <span className="text-[#dfe2eb] text-sm sm:text-base font-bold uppercase">
                    {user.firstname?.[0]}
              ...
              {isProfileOpen && user && (
                <div className="absolute right-0 top-[calc(100%+10px)] w-56 bg-[#1c2026] border border-[#3b4a44] rounded-xl shadow-2xl py-2 z-[60] animate-in fade-in zoom-in duration-150">
                  ...
                  <button
                    onClick={handleLogout}
                    className="w-full text-left px-4 py-2 text-[#ffb4ab] hover:bg-[#262a31] text-sm transition-colors flex items-center gap-2 font-medium focus:outline-none"
                  >
                    <span className="material-symbols-outlined text-[18px]">
                      logout
                    </span>{" "}
