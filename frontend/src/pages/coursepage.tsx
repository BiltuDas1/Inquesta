...
const ENROLLMENT_LINK =
  "https://docs.google.com/forms/d/e/1FAIpQLScmHoRcvkyw7EEZYQIX-c5F6qwwb9VuEprHzjL3vPvkRlEI1Q/viewform?fbzx=-4420388597224411814";
...
function CourseCard({
  ...
  const normalizedLevel = (course.level.charAt(0).toUpperCase() +
    course.level.slice(1).toLowerCase()) as Course["level"];
  ...
  return (
    <div className="group bg-[#1c2026] rounded-[24px] overflow-hidden border border-[#262a31] transition-all duration-300 hover:-translate-y-2 hover:border-[#6fffd9] hover:shadow-[0_20px_40px_-15px_rgba(111,255,217,0.15)] flex flex-col h-full relative">
      ...
      <div className="absolute top-4 left-4 z-10">
        <span
          className={`px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-widest border backdrop-blur-md ${levelStyles[normalizedLevel]}`}
        >
          {course.level}
        </span>
      </div>
      ...
      <div className="p-6 flex flex-col flex-1">
        ...
        <div className="flex items-center gap-1.5 mb-2 text-[#84948e] text-[10px] font-bold uppercase tracking-[0.1em]">
          <svg
            className="w-3.5 h-3.5 text-[#6fffd9]/70"
            fill="none"
            stroke="currentColor"
            viewBox="0 0 24 24"
          >
        ...
        <div className="mt-auto flex justify-between items-center pt-5 border-t border-[#262a31]">
          ...
          <span
            className={`text-xl font-extrabold font-['Plus_Jakarta_Sans'] ${course.price === 0 ? "text-[#ffb800]" : "text-[#6fffd9]"}`}
          >
            {course.price === 0 ? "Free" : `₹ ${course.price.toLocaleString()}`}
          </span>
          <button
            onClick={() => window.open(ENROLLMENT_LINK, "_blank")}
            className="bg-[#6fffd9] text-[#00382c] rounded-xl px-5 py-2 text-[12px] font-bold hover:bg-[#00e5bc] transition-all"
          >
            Enroll
          </button>
        </div>
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-[#1c2026] border border-[#262a31] w-full max-w-xl max-h-[85vh] rounded-[32px] overflow-hidden shadow-2xl flex flex-col animate-in zoom-in-95 duration-300">
        <button
          onClick={onClose}
          className="absolute top-6 right-6 z-10 text-[#84948e] hover:text-[#6fffd9] transition-colors"
        >
          <svg
            className="w-6 h-6"
            fill="none"
        ...
        <div className="p-8 md:p-10 flex flex-col flex-1 overflow-hidden">
          ...
          <div className="mt-8 flex flex-col sm:flex-row items-center justify-between gap-6 border-t border-[#262a31] pt-8">
            <div className="text-center sm:text-left">
              <span className="text-[#84948e] text-[11px] uppercase font-bold tracking-[0.15em] mb-1 block">
                Course Price
              </span>
            ...
            <button
              onClick={() => window.open(ENROLLMENT_LINK, "_blank")}
              className="w-full sm:w-auto bg-[#6fffd9] text-[#00382c] rounded-2xl px-10 py-4 font-bold hover:bg-[#00e5bc] transition-all active:scale-95"
            >
              Enroll Now
            </button>
          </div>
    ...
    <div className="bg-[#10141a] min-h-screen py-16 px-6 relative font-['Inter']">
      ...
      <div className="max-w-[1200px] mx-auto">
        <header className="text-center mb-16">
          ...
          <p className="text-[#b9cac3] text-lg">
            Premium industry-ready courses designed for success.
          </p>
        </header>
