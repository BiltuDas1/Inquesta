  ...
  return (
    <nav className="fixed top-0 w-full z-50 bg-background/80 backdrop-blur-xl border-none shadow-2xl shadow-black/40">
      <div className="flex justify-between items-center px-8 py-4 max-w-7xl mx-auto font-headline tracking-tight relative">
        <Link to="/" className="text-2xl font-bold tracking-tighter text-on-surface flex gap-2">
        ...
        >
          <Link className="text-on-surface-variant hover:text-on-surface transition-colors" to="/courses">
            Courses
          </Link>
          <Link className="text-on-surface-variant hover:text-on-surface transition-colors" to="/contact">
            Contact
          </Link>
          ...
          <div className="flex flex-col lg:flex-row items-center space-y-4 lg:space-y-0 lg:space-x-6 w-full lg:w-auto">
            {!user ? (
              <>
                ...
                <button onClick={() => navigate("/register")} className="bg-gradient-to-br from-primary to-primary-container text-on-primary px-6 py-2.5 rounded-full font-bold text-sm active:scale-95 transition-transform shadow-lg shadow-primary-container/20 glow-hover w-full lg:w-auto">
                  Get Started
                </button>
              </>
            ) : (
              <div className="relative w-full lg:w-auto flex justify-center" ref={profileRef}>
                <button onClick={() => setIsProfileOpen(!isProfileOpen)} className="w-10 h-10 rounded-full bg-[#343d96] text-[#bdc2ff] font-bold text-sm border-2 border-[#3b4a44] hover:border-[#6fffd9] cursor-pointer flex items-center justify-center transition-colors shrink-0">
                ...
                {isProfileOpen && (
                  <div className="absolute lg:right-0 top-[calc(100%+10px)] w-56 bg-[#1c2026] border border-outline-variant rounded-xl shadow-2xl py-2 z-[60] animate-in fade-in zoom-in duration-150">
                    ...
                    <button onClick={handleLogout} className="w-full text-left px-4 py-2 text-[#ffb4ab] hover:bg-surface-container text-sm transition-colors flex items-center gap-2 font-medium">
